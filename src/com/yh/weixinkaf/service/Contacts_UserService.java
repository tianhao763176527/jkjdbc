...
import com.yh.weixinkaf.pojo.User;
import com.yh.weixinkaf.test.UserTest;
...
public class Contacts_UserService {
	...
	public void createUser(String accessToken,User user) {
		...
		if (null != jsonObject) {  
			if (0 != jsonObject.getInteger("errcode")) {
				log.error("创建成员失败 errcode:{} errmsg:{}", jsonObject.getInteger("errcode"), jsonObject.getString("errmsg"));
			}
		}
	}
	...
	public void getDepartmentUserDetails(String accessToken,String departmentId,String fetchChild) {
		...
		if (null != jsonObject) {
			if (0 != jsonObject.getInteger("errcode")) {
				log.error("获取部门成员详情失败 errcode:{} errmsg:{}", jsonObject.getInteger("errcode"), jsonObject.getString("errmsg"));
			}  
		}  
	}
